...
void memberViewLoans::returnButtonClicked(){
    ...
    qDebug() << num << "\n";
    if (sysLib.isLoaned(userId, book[indexPos[num]].getBookId())) {
        sysLib.returnBook(userId, book[indexPos[num]].getBookId());
        QMessageBox::information(this, "Book Returned", book[indexPos[num]].getBookName() + " has been successfully returned!");
    } else if (!sysLib.isPreBook(userId, book[indexPos[num]].getBookId()) && book[indexPos[num]].getStock() == 0 && book[indexPos[num]].getAvailStatus() == 0) {
        PreOrderBook preOrder;

        preOrder.setPreBookID(sysLib.generateID(8));
        preOrder.setPreBook_BookID(book[indexPos[num]].getBookId());
        ...
        if (book[indexPos[num]].getAvailStatus() != 0 && book[indexPos[num]].getStock() > 0 && !sysLib.isPreBook(userId, book[indexPos[num]].getBookId())) {
            bookItem.setBookItemID(sysLib.generateID(7));
            bookItem.setBookItem_MemberID(userId);
            bookItem.setBookItem_BookID(book[indexPos[num]].getBookId());
            sysLib.loanBook(bookItem);
            QMessageBox::information(this, "Loaned", "You have loaned " + book[indexPos[num]].getBookName() + " for 7 days.");
        }
    }
...
void memberViewLoans::viewButtonClicked(){
   ...
   // QMessageBox::information(this,"Button",QString::number(num) + " From button 2");
    ...
    connect(bookdetails, SIGNAL(goBack()), this, SLOT(deleteRecords()));
    bookdetails->show();
...
void memberViewLoans::on_bcktoCatologueBtn_clicked()
{
    emit showCatalogue();
    hide();
}
